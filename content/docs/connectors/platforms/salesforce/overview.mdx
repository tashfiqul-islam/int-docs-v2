...
<TypeTable
  type={{
    "Work Order Creation": {
      type: "Inbound",
      description: "Create FN work order from Salesforce record via Flow trigger"
    },
    "Work Order Update": {
      type: "Inbound",
      description: "Update existing FN work order when Salesforce record changes"
    },
    "Status Synchronization": {
      type: "Outbound",
      description: "Update Salesforce record when FN work order status changes"
    },
    "Message Sync": {
      type: "Bidirectional",
      description: "Sync comments via CaseComment or similar objects"
    }
  }}
/>
