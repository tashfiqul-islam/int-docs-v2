...
<TypeTable
  type={{
    "$.input": {
      type: "object",
      description: "Complete input payload containing work order or external record data. Access nested fields with dot notation: `$.input.workorder.id`",
      required: true
    },
    "$.util": {
      type: "object",
      description: "Utility library with helper functions for common transformations (field lookup, date conversion, mapping)",
      required: true
    },
    "std": {
      type: "object",
      description: "JSONNET standard library with array, string, math, and type functions",
      required: true
    }
  }}
/>
...
<Accordions type="multiple">
  ...
  <Accordion title="Null Pointer Errors">
