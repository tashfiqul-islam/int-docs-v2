...
<TypeTable
  type={{
    "x-fn-signature": {
      type: "string",
      description: "HMAC-SHA256 signature for verifying webhook authenticity. Format: `sha256=<hex_digest>`",
      required: true
    },
    "x-fn-webhook-id": {
      type: "string",
...
<TypeTable
  type={{
    "eventName": {
      type: "string",
      description: "Name of the event that triggered the webhook (e.g., `workorder.created`, `workorder.status.published`)",
      required: true
    },
    "eventId": {
      type: "string",
      description: "Unique identifier for this event. Use for idempotency—processing the same event multiple times should have the same effect as processing it once",
      required: true
    },
    "workOrderId": {
      type: "number",
      description: "Field Nation work order identifier associated with this event",
      required: true
    },
    "timestamp": {
      type: "string",
      description: "ISO 8601 timestamp when the event occurred (not when delivered—slight delay possible)",
      required: true
    },
    "data": {
      type: "object",
      description: "Complete work order object with all current details. Structure matches REST API WorkOrder model",
      required: true
    }
  }}
/>
...
The `data` field contains the complete work order object, which matches the structure returned by the [REST API](/docs/rest-api/v2/introduction).

### Key Fields Overview

<Tabs items={["Core Fields", "Schedule & Location", "Financial", "Participants"]}>
  <Tab value="Core Fields">
    ```json
    {
      "id": 12345,
      "title": "Router Installation - Site 42",
...
<Callout type="info">
**Full Schema**: The complete work order schema includes 100+ fields. Refer to the [REST API documentation](/docs/rest-api/v2/introduction) for the full specification.
</Callout>
